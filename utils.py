...
class GOF(object):
    ...
    def index_of_agreement(self):
        ...
        denominator = np.sum(np.square(
            (np.abs(self.sim-np.mean(self.sim)))+(np.abs(self.obs-np.mean(self.obs)))))
    ...
    def save_gof(self):
        ...
        col_names = ["Mean Error", "Percent Bias", "Mean Absolute Error",
                     "Root Mean Square Error", "Nash-Shutcliffe-Efficiency", "Index of Agreement"]
